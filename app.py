...
if seleccion == "🏠 Inicio":
    ...
    with c2: st.markdown("<div class='step-card' style='text-align:center;'><div style='font-size:2rem;'>🔍</div><strong>2. Analisis automatico</strong><br><span style='font-size:0.82rem;color:#718096;'>5 metricas calculadas mes a mes para cada par</span></div>", unsafe_allow_html=True)
    with c3: st.markdown("<div class='step-card' style='text-align:center;'><div style='font-size:2rem;'>🚦</div><strong>3. Semaforo de riesgo</strong><br><span style='font-size:0.82rem;color:#718096;'>Verde Seguro · Amarillo Precaucion · Rojo No operar</span></div>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    metricas = [
        ...
        ("📈 ADX — Average Directional Index",
         ...
         "ADX = 15 → mercado sin direccion clara → Grid seguro ✅\nADX = 45 → tendencia fuerte en marcha → Grid en peligro 🔴",
        ...
        ("🕯️ Porcentaje de velas en la misma direccion",
         ...
         "Un mes de 22 dias: 17 bajistas y 5 alcistas → 77% bajistas → tendencia clara 🔴\nUn mes con 12 bajistas y 10 alcistas → 55% → equilibrio 🟢",
    ...
    st.markdown("""<div class='step-card'>
        <div style='font-weight:700;font-size:0.95rem;margin-bottom:1rem;'>🚦 Como se combinan en el semaforo</div>
        ...
        <strong>🔴 ROJO (8-15 puntos):</strong> Tendencia fuerte historica. No encender el robot este mes.
        </div></div>""", unsafe_allow_html=True)

elif seleccion == "📊 Analisis de pares":
    page_header("Analisis de Pares","Sube tus datos y obtén el semaforo de riesgo","📊")
    st.markdown("""<div class='step-card'>
    ...
    if not archivos:
        ...
        if "gr_res" in st.session_state:
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown("<div class='step-title'>🚦 Semaforo por mes y par</div>", unsafe_allow_html=True)
            ...
            for par in pares:
                ...
                with st.expander(f"📊 {par}", expanded=True):
                    ...
                    with c3: st.markdown(f"<div class='sem-rojo'>🔴 NO OPERAR ({len(mr)})<br><small>{', '.join(mr) or 'Ninguno'}</small></div>", unsafe_allow_html=True)
                    st.markdown("<br>", unsafe_allow_html=True)
                    ...
                    if ma: st.markdown(f"<div style='background:#FFFBEA;border-left:4px solid #FFD166;padding:1rem;border-radius:8px;margin:0.5rem 0;'>⚠️ <strong>Reduce el lotaje a la mitad en:</strong> {', '.join(ma)}<br><small>Hay algo de direccion. Opera con mas margen.</small></div>", unsafe_allow_html=True)
                    if mr: st.markdown(f"<div class='alert-fail'>🚫 <strong>No enciendas el robot en:</strong> {', '.join(mr)}<br><small>Tendencia fuerte historica. Alto riesgo de quema.</small></div>", unsafe_allow_html=True)
                    st.markdown("<br>", unsafe_allow_html=True)
            ...
            if len(pares) > 1:
                st.markdown("<br>", unsafe_allow_html=True)
                st.markdown("<div class='step-title'>⚖️ Comparativa entre pares</div>", unsafe_allow_html=True)
                comp = []
                for par in pares:
                ...
                st.markdown(f"<div class='alert-ok'>🏆 <strong>{mejor}</strong> es el par mas seguro para Grid con {df_comp.iloc[0]['🟢 Meses seguros']} meses verdes al año. <strong>{peor}</strong> es el mas peligroso con {df_comp.iloc[-1]['🔴 Peligrosos']} meses rojos.</div>", unsafe_allow_html=True)
